...
#pragma pack(pop, 1)
	...
	data.resize_storage({ height, width, n_planes_out });
		...
		for (size_t j = 0; j < width; ++j)
		{
			if (grayscale && n_planes_in != n_planes_out)
			{
				...
				for (uint32_t plane = 0; plane < n_color_planes; ++plane)
					val += read_byte();
				data(row, j, 0) = val * grayscale_normalizer;

				if (n_color_planes != n_planes_in)
					(void)read_byte();
			}
			else
			{
				for (uint32_t plane = 0; plane < n_planes_out; ++plane)
					data(row, j, bgr_to_rgb_mapper[plane]) = read_byte() / (fp_t)255;
			}
		}
